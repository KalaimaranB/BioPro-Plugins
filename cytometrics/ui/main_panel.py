...
from PyQt6.QtWidgets import (
    ...
    QCheckBox, QDoubleSpinBox, QProgressBar, QTabWidget, QHBoxLayout
)
...
import numpy as np
import cv2
from .workers import PipelineWorker, ModelDownloadWorker

from biopro.ui.theme import Colors
from .image_canvas import MultiChannelCanvas
from .channel_manager import ChannelManagerWidget
from .workers import PipelineWorker  # <--- NEW IMPORT
...
import csv
from PyQt6.QtWidgets import QFileDialog # Add this to your QtWidgets import list
from PyQt6.QtGui import QPainter, QBrush, QColor # Add QPainter, QBrush to your QtGui import list


class HardwareMonitor(QWidget):
    """Live system telemetry plot to track AI computation loads over time."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFixedHeight(240)  # Taller to support stacked plots
        layout = QVBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)

        self.max_ticks = 60
        self.sys_cpu_data = collections.deque([0] * self.max_ticks, maxlen=self.max_ticks)
        self.app_cpu_data = collections.deque([0] * self.max_ticks, maxlen=self.max_ticks)
        self.vram_data = collections.deque([0] * self.max_ticks, maxlen=self.max_ticks)
        self.x_data = list(range(self.max_ticks))
        ...
        # --- STACKED SUBPLOTS ---
        self.ax1 = self.fig.add_subplot(211)  # Top Plot
        ...
        self.ax1.set_ylabel('CPU %', color='#9CA3AF', fontsize=9)

        # Bottom Plot: VRAM
        self.ax2.set_ylim(0, 500)
        self.ax2.set_ylabel('VRAM (MB)', color='#F59E0B', fontsize=9)

        # Draw Lines
        self.line_sys, = self.ax1.plot(self.x_data, self.sys_cpu_data, color='#374151', lw=1, linestyle='--',
                                       label='Mac CPU')
        self.line_app, = self.ax1.plot(self.x_data, self.app_cpu_data, color='#3B82F6', lw=2, label='App Compute')
        self.line_vram, = self.ax2.plot(self.x_data, self.vram_data, color='#F59E0B', lw=2, label='AI Memory')

        # Legend and Margins
        self.fig.legend(loc='upper center', bbox_to_anchor=(0.5, 0.98), ncol=3, frameon=False, fontsize=8,
                        labelcolor='#E5E7EB')
        self.fig.subplots_adjust(top=0.85, bottom=0.05, left=0.15, right=0.95, hspace=0.15)
    ...
    def update_stats(self):
        try:
            ...
            app_cpu = min(100.0, self.process.cpu_percent() / self.cpu_count)
            ...
            self.line_sys.set_ydata(self.sys_cpu_data)
            self.line_app.set_ydata(self.app_cpu_data)
            self.line_vram.set_ydata(self.vram_data)

            self.ax2.set_ylim(0, max(500, max(self.vram_data) * 1.2))
            self.canvas.draw()
        except Exception:
            pass

class ModelManagerDialog(QDialog):
    """A dedicated manager to download or delete the 100MB AI model."""
    ...
    def __init__(self, parent=None):
        ...
        self.progress_bar.setStyleSheet(f"""
            QProgressBar {{ background: {Colors.BG_DARK}; border: 1px solid {Colors.BORDER}; border-radius: 4px; text-align: center; color: white; }}
...
class CytoMetricsPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        from PyQt6.QtWidgets import QHBoxLayout  # Ensure this is imported
        ...
        self.progress_bar.setVisible(False)
        self.progress_bar.setStyleSheet(f"""
            QProgressBar {{ background: {Colors.BG_DARKEST}; border: 1px solid {Colors.BORDER}; border-radius: 4px; color: transparent; height: 12px; }}
            QProgressBar::chunk {{ background-color: {Colors.ACCENT_PRIMARY}; border-radius: 3px; }}
        """)
        ...
        self.tabs.addTab(tab_detect, "2. Smart Detect")
    ...
    def _render_composite(self):
        ...
        self._update_dropdowns()  # <--- Use the new helper here
